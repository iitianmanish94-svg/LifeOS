...
def study():
    if request.method == "POST":
        db.session.add(StudyLog(user_id=current_user.id, subject=request.form["subject"],
                                ...
                                notes=request.form.get("notes", ""), logged_on=date.today()))
...
def weight():
    if request.method == "POST":
        item = WeightLog.query.filter_by(user_id=current_user.id, logged_on=date.today()).first()
        if item: item.weight = float(request.form["weight"])
        else: db.session.add(WeightLog(user_id=current_user.id, weight=float(request.form["weight"])))
